...
/**
 * Quality attachments upload array
 * Multiple purchase attachments can be upload if input type is array or dropzone plugin is used
 * @param  mixed $related related
 * @param  mixed $id id
 * @param  string $index_name attachments index, in different forms different index name is used
 * @return mixed
 */
function handle_quality_attachments_array($label, $id, $index_name = 'attachments')
{
    if (!is_dir(get_upload_path_by_type('quality'))) {
        mkdir(get_upload_path_by_type('quality'), 0755);
    }
    if (!is_dir(get_upload_path_by_type('quality') . $label)) {
        mkdir(get_upload_path_by_type('quality') . $label, 0755);
    }
    $uploaded_files = [];
    $path           = get_upload_path_by_type('quality') . $label . '/'. $id . '/';
    $CI             = &get_instance();
...
function get_upload_path_by_type($type)
{
    ...
    switch ($type) {
        ...
        break;
        case 'quality':
            $path = QUALITY_ATTACHMENTS_FOLDER;
