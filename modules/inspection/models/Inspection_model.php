...
class Inspection_model extends App_Model
{
    ...
    public function add_perform_inspection($data, $label, $id)
    {
        ...
        for($i = 1; $i < 30; $i++) {
            if(isset($_FILES['attachment'.$i])) {
                $uploadedFiles = handle_quality_attachments_array($label, $id, 'attachment'.$i);
                if ($uploadedFiles && is_array($uploadedFiles)) {
                    foreach ($uploadedFiles as $file) {
                        ...
                        $attach['dateadded'] = date('Y-m-d H:i:s');
                        $this->db->insert(db_prefix() . $label. '_files', $attach);
                    }
                }
            }
        }
    ...
    public function get_checklist_data($label, $id)
    {
        if (is_numeric($id)) {
            ...
            $checklist_data = $this->db->get(db_prefix() . $label)->result_array();
            if(!empty($checklist_data)) {
                $this->db->where('inspection_id', $id);
                $attachments = $this->db->get(db_prefix() . $label. '_files')->result_array();
                $checklist_data[0]['attachments'] = $attachments;
            }
