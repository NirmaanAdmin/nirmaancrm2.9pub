...
class Migration_Version_119 extends App_module_migration
{
    public function up()
    {
        ...
        if ($CI->db->table_exists($table) && $CI->db->field_exists('feedback_comment', $table)) {
            $CI->db->query(
                "ALTER TABLE {$table} 
                 ADD `recurring` INT NOT NULL DEFAULT '0' AFTER `feedback_comment`,
                 ADD `recurring_type` VARCHAR(10) NULL DEFAULT NULL AFTER `recurring`,
                 ADD `repeat_every` INT NULL DEFAULT NULL AFTER `recurring_type`,
                 ADD `custom_recurring` TINYINT NOT NULL AFTER `recurring_type`, 
                 ADD `cycles` INT NOT NULL DEFAULT '0' AFTER `custom_recurring`, 
                 ADD `total_cycles` INT NOT NULL DEFAULT '0' AFTER `cycles`,
                 ADD `last_recurring_date` DATE NULL DEFAULT NULL AFTER `total_cycles`;
          ");
        }
    }
