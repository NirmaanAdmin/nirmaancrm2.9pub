...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'subject') {
            $_data = '<a href="' . admin_url('quality/goal/' . $aRow['id']) . '">' . $_data . '</a>';
            $_data .= '<div class="row-options">';
            $_data .= '<a href="' . admin_url('quality/goal/' . $aRow['id']) . '">' . _l('view') . '</a>';

            if (has_permission('goals', '', 'delete')) {
                $_data .= ' | <a href="' . admin_url('quality/delete/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
            }
            $_data .= '</div>';
    ...
    ob_start();
    $achievement          = $this->ci->quality_model->calculate_goal_achievement($aRow['id']);
