...
class Quality extends AdminController
{
    ...
    public function goal($id = '')
    {
        ...
        if ($this->input->post()) {
            if ($id == '') {
                ...
                $id = $this->quality_model->add($this->input->post());
                if ($id) {
                    set_alert('success', _l('added_successfully', _l('goal')));
                    redirect(admin_url('quality/goal/' . $id));
                }
            } else {
                if (!has_permission('goals', '', 'edit')) {
                    access_denied('goals');
                }
                $success = $this->quality_model->update($this->input->post(), $id);
                if ($success) {
        ...
        if ($id == '') {
            ...
            $data['goal']        = $this->quality_model->get($id);
            $data['achievement'] = $this->quality_model->calculate_goal_achievement($id);
    ...
    public function delete($id)
    {
        ...
        $response = $this->quality_model->delete($id);
        if ($response == true) {
    ...
    public function notify($id, $notify_type)
    {
        ...
        $success = $this->quality_model->notify_staff_members($id, $notify_type);
        if ($success) {
